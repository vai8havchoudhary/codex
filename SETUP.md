...
Preflight is configuration admission, not native delegation proof. Luna establishes two read-only Grok responses before writing and retains one for final review. Grok gathers source and uses fresh single-turn Gemini plan criticism before edits, then a distinct fresh final reviewer after validation. Gemini reviewed supplied evidence only: complete bounded initial packets, no tools, READY reservation, send_input, follow-ups or history reuse. Each semantic gate permits one primary attempt plus at most one fresh transport retry; exhaustion blocks. Record packet SHA-256/native references, and stop if the complete final diff and relevant full resulting files cannot fit. After a long run, audit actual native spawn IDs, model overrides, returned final verdict, and final revision. Schema-2 checkpoint witness validation does not authenticate self-reported model claims. Historical schema-1 and shape-valid schema-2 records retain read/digest compatibility; new Gemini writes enforce reviewer freshness across the run.
